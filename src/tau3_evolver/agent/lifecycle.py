...
from tau3_evolver.agent.factory import audit_marker, render_observation
from tau3_evolver.execution.events import ExecutionContext
from tau3_evolver.agent.decisions import WriteDecision
from tau3_evolver.agent.prompts import build_write_prompt
...
from tau3_evolver.memory.outcomes import EpisodeMemoryPolicy, classify_episode_memory
    ...
    *,
    ...
    if propose_experience:
        ...
        if memory_policy.should_generate:
            ...
            prompt_tokens, completion_tokens, token_usage_complete = _accumulate_token_usage(
                prompt_tokens,
                completion_tokens,
                token_usage_complete,
                write_audit,
            )
            accepted, dropped_by_tier = _apply_write_quotas(write_decision, config)
            learned = [
                _learned_proposal(
                    memory,
                    task_id=task_id,
...
def _emit(
    context: ExecutionContext, task_id: str, event_type: str, **payload: Any
) -> None:
