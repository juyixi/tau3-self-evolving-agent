...
from tau3_evolver.slow_loop.task_grouping import GROUPING_REVISION
...
def _materialize(request: DatasetBuildRequest) -> _MaterializedDataset:
    ...
    config = config.model_copy(
        update={
            "tau2": config.tau2.model_copy(
                update={"repo_path": _resolve_from_project(config.tau2.repo_path, project)}
            )
        }
    )
    source_paths = tuple(
    ...
    benchmark = _source_benchmark(source_paths)
...
def _build_manifest(
    ...
    *,
    ...
    return {
        ...
        "policy_lineage": {
            "memory_generation": ledger.memory_generation,
            "model_revision": ledger.model_revision,
            "adapter_revision": ledger.adapter_revision,
            "runtime_revision": ledger.runtime_revision,
        },
        "official_split": {
            "name": "train",
        ...
        "memory": {
            "namespace": ledger.memory_namespace,
        ...
        "audit": {"schema_version": 1, "passed": True},
...
def _source_benchmark(paths: Sequence[Path]) -> str:
    benchmarks: set[str] = set()
    for path in paths:
        try:
            run = json.loads((path / "run.json").read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            raise ValueError(f"unable to identify source benchmark: {path}") from error
        execution = run.get("execution") if isinstance(run, dict) else None
        benchmark = execution.get("benchmark") if isinstance(execution, dict) else None
        if not isinstance(benchmark, str) or not benchmark.strip():
            raise ValueError(f"source run has no benchmark: {path}")
        benchmarks.add(benchmark.strip())
    if len(benchmarks) != 1:
        raise ValueError("source runs must use one benchmark")
