...
from tau3_evolver.fast_loop.contracts import maintenance_command_schemas
...
def project_public_context(
    *,
    ...
    normalized_policy = _json_copy(redact_public_data(policy), "policy")
    normalized_tools = _json_copy(
        redact_public_data([normalize_tool_schema(tool) for tool in tools]),
        "tools",
    )
    _reject_forbidden_fields(normalized_policy, "policy")
    _reject_forbidden_fields(normalized_tools, "tools")
    context = {
        "task_instruction": redact_public_data(task_instruction).strip(),
        "policy": normalized_policy,
        "tools": normalized_tools,
        "observation": redact_public_data(observation).strip(),
