...
def _execute_round(
    *,
    ...
    try:
        ...
        diagnostics = redact_public_data(diagnostics)
        ...
        decision, decision_audit = generate_decision(
            policy,
            prompt,
            MaintenanceDecision,
            validator=lambda proposed: _validate_commands(
                proposed,
                {
                    item["id"]
                ...
                {
                    item["id"]
                    for item in public_diagnostics["tip"]["items"]
                },
                requires_tip_reduction,
            ),
            label="maintenance",
        )
        repair_used = decision_audit["repaired_output"] is not None
...
def _emit(
    ...
    **payload: Any,
    ...
    context.event_writer.append(
        redact_public_data(context.event(event_type, task_key, **payload))
    )
