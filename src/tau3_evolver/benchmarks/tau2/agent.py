...
from tau3_evolver.benchmarks.tau2.actions import assistant_for_action
from tau3_evolver.benchmarks.tau2.agent_state import Tau3AgentState
from tau3_evolver.benchmarks.tau2.runtime import Tau2RuntimeBinding
from tau3_evolver.benchmarks.tau2.action_codec import (
    TAU2_STOP_ACTION,
    Tau2ActionCodec,
)
...
from tau3_evolver.fast_loop.decision import generate_decision
from tau3_evolver.fast_loop.prompts import (
    build_action_prompt,
    build_selection_prompt,
    project_public_context,
)
from tau3_evolver.fast_loop.selection import (
    ...
    *,
    ...
    class Tau3Agent(runtime.half_duplex_agent_type):
        ...
        def generate_next_message(
            ...
            available_tool_names = _tool_names(self._public_tools)
            action, audit = generate_decision(
                policy,
                action_prompt,
                ActionDecision,
                validator=lambda decision: Tau2ActionCodec.decode(
                    decision.action,
        ...
        def _start(
            ...
            selection, audit = generate_decision(
                policy,
                selection_prompt,
                SelectionDecision,
                candidate_ids=[candidate.memory_id for candidate in compatible],
                validator=lambda decision: validate_selection_limits(
            ...
            return {
                **public_context,
                ...
                "query_hash": compatible[0].query_hash if compatible else query_hash(query),
                "retriever_revision": (
                    compatible[0].retriever_revision
                    if compatible
                    else retriever.provider.model_revision
                ),
                "candidates": [candidate_evidence(item) for item in compatible],
                "selected": [candidate_evidence(item) for item in state.selected],
