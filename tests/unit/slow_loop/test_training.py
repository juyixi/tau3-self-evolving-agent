...
from tau3_evolver.slow_loop.alignment import render_public_prompt
from tau3_evolver.slow_loop.examples import (
    OPD_DATASET_SCHEMA_VERSION,
    OPD_SAMPLE_UNIT_CONTRACT,
    OPDExample,
)
from tau3_evolver.slow_loop.opd_step import OPDStepResult
from tau3_evolver.slow_loop import training as trainer_module
...
def test_effective_batch_and_final_partial_average_each_optimizer_window(
    ...
    monkeypatch.setattr(
        "tau3_evolver.slow_loop.training.shared_policy_opd_step",
...
def test_checkpoint_contains_adapter_optimizer_and_atomic_json_manifests_only(
    ...
    monkeypatch.setattr("tau3_evolver.slow_loop.training.os.replace", record_replace)
    result = OPDTrainer(
        ToyPolicy(),
        ToyTokenizer(),
        _config(),
