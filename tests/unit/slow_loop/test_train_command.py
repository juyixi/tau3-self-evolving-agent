...
from tau3_evolver.slow_loop import train_command as train_opd_lora
from tau3_evolver.slow_loop.audit import AuditError, AuditReport
from tau3_evolver.slow_loop.examples import (
    OPD_DATASET_SCHEMA_VERSION,
    OPD_SAMPLE_UNIT_CONTRACT,
)
from tau3_evolver.slow_loop.training import TrainingRequest
...
def test_importing_training_cli_does_not_import_qwen_loader() -> None:
    module_names = (
        "tau3_evolver.slow_loop.train_command",
        "tau3_evolver.models",
        "tau3_evolver.models.lora",
        "tau3_evolver.models.qwen35",
    )
    saved_modules = {
    ...
    try:
        importlib.import_module("tau3_evolver.slow_loop.train_command")

        assert "tau3_evolver.models.qwen35" not in sys.modules
    finally:
        for name in module_names:
