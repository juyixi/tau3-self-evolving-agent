...
def test_runs_two_tasks_with_canonical_auditable_event_order_and_closes_environments() -> None:
            ...
            step_results=[
                StepResult(
                    ...
                    info={
                        ...
                        "evaluator": {"criteria": [{"name": "policy", "score": 0.75}]},
                    },
                )
            ],
        ),
            ...
            step_results=[
                ...
                StepResult(
                    ...
                    info={
                        ...
                        "evaluator_details": {"complete": True, "explanation": "all checks passed"},
                    },
                ),
            ],
        ),
    ...
    assert finished[0]["reward_info"] == {"total_reward": 0.75, "policy": 0.75}
    assert finished[0]["evaluator_details"] == {"criteria": [{"name": "policy", "score": 0.75}]}
    assert finished[1]["final_reward"] == 1.0
    assert finished[1]["reward_info"] == {"total_reward": 1.0, "policy": 1.0}
    assert finished[1]["evaluator_details"] == {"complete": True, "explanation": "all checks passed"}
    ...
    environment = FakeEnvironment(
        ...
        step_results=[
            StepResult(
                ...
                terminated=True,
                truncated=False,
                info={"reward_info": {"score": 1.0}, "evaluator": {"access_token": "secret-value"}},
            )
        ],
    )
    events = EventCollector()
