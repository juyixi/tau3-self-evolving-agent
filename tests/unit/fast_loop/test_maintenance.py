...
from tau3_evolver.memory.operations import DeleteCommand
...
from tau3_evolver.memory.types import MemoryStatus, MemoryTier
...
def _context(events: EventCollector, **overrides: Any) -> ExecutionContext:
    values = {
        ...
        "checkpoint": "adapter-4",
        "memory_source_namespace": "retail",
...
def test_state_write_failure_leaves_round_retryable_after_command_commit(
    ...
    from tau3_evolver.fast_loop import maintenance
...
def test_nonlearning_context_is_rejected_before_state_or_policy_access(
    tmp_path: Path, split: str, mode: str
) -> None:
    repository = MemoryRepository(tmp_path / "memory")
    ...
    with pytest.raises(ValueError, match="train split and train mode"):
